...
class BunnyAPI
{
    private const API_KEY = '7bc02500-d0ed-40e4-9af8-ec110b35b0933cce86db-6da5-4328-951a-162d09692d70'; //BunnyCDN API key
    ...
    private const STREAM_LIBRARY_ACCESS_KEY = 'c917b5bc-895e-4f58-a2b8b8314d28-bf45-4549';
    ...
    public string $stream_video_guid = '';
    ...
    private function APIcall(string $method, string $url, array $params = [], bool $storage_call = false, bool $video_stream_call = false): string
    {
        ...
        if (!$storage_call && !$video_stream_call) { //General CDN pullzone
            ...
            curl_setopt($curl, CURLOPT_HTTPHEADER, array("Accept: application/json", "Content-Type: application/json", "AccessKey: " . self::STREAM_LIBRARY_ACCESS_KEY . ""));
        } else { //Storage zone
    ...
    public function convertBytes(int $bytes, string $convert_to = 'GB', bool $format = true, int $decimals = 2): int
    {
        if ($convert_to === 'GB') {
    ...
    public function monthChargeBreakdown(): array
    {
        ...
        return array(
            'storage' => $ar['MonthlyChargesStorage'], 'EU' => $ar['MonthlyChargesEUTraffic'],
    ...
    public function dirSize(string $dir = ''): array
    {
        ...
        return array(
            'dir' => $dir, 'files' => $files, 'size_b' => $size, 'size_kb' => number_format(($size / 1024), 3),
    ...
    public function listFiles(string $location = ''): string
    {
        ...
        foreach ($array as $value) {
            if ($value['IsDirectory'] === false) {
                ...
                $guid = $value['Guid'];
                $items['data'][] = array(
                    'name' => $file_name, 'file_type' => $file_type, 'size' => $size_kb, 'created' => $created,
    ...
    public function listFolders(string $location = ''): string
    {
        ...
        foreach ($array as $value) {
            ...
            if ($value['IsDirectory'] === true) {
                $items['data'][] = array(
                    'name' => $foldername, 'created' => $created,
                    'last_changed' => $last_changed, 'guid' => $guid
                );
            }
        }
    ...
    public function listAll(string $location = ''): string
    {
        ...
        foreach ($array as $value) {
            ...
            $items['data'][] = array(
                'name' => $file_name, 'file_type' => $file_type, 'size' => $size_kb, 'is_dir' => $value['IsDirectory'], 'created' => $created,
                'last_changed' => $last_changed, 'guid' => $guid
            );
        }
        return json_encode($items);
    ...
    */
    ...
    {
        if (!empty($collection_guid)) {
            return $this->APIcall('POST', "library/$library_id/videos?title=$video_title&collectionId=$collection_guid", ["title" => $video_title], false, true);
        } else {
            return $this->APIcall('POST', "library/$library_id/videos?title=$video_title", ["title" => $video_title], false, true);
        }
    }
