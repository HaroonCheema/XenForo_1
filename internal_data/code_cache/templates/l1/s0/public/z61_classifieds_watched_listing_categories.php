<?php
// FROM HASH: 2d897dabcc200b0c00d6f86177443e12
return array(
...
';
			...
			foreach ($__compilerTemp2 AS $__vars['id'] => $__vars['treeEntry']) {
				...
				if ($__vars['categoryWatch']) {
					$__compilerTemp1 .= '
                        ';
					$__compilerTemp3 = '';
					if ($__vars['categoryWatch']['notify_on'] == 'classifieds_listing') {
						$__compilerTemp3 .= '<li>' . 'New classifieds listings' . '</li>';
					}
					$__compilerTemp4 = '';
					...
					$__vars['bonusInfo'] = $__templater->preEscaped('
                        ...
                        ');
					$__compilerTemp1 .= '
                        ' . $__templater->callMacro('z61_classifieds_category_list_macros', 'category', array(
    ...
    ', array(
			'action' => $__templater->func('link', array('watched/classifieds-categories/update', ), false),
			'ajax' => 'true',
			'class' => 'block',
			'autocomplete' => 'off',
		)) . '
