...
';
	...
	if (!$__templater->method($__vars['forum'], 'isInsert', array())) {
		$__compilerTemp5 .= '
	';
		if ($__templater->method($__vars['node'], 'getIconUnread', array())) {
			$__compilerTemp5 .= '
		' . $__templater->formRow('
			<img src="' . $__templater->func('base_url', array($__templater->method($__vars['node'], 'getIconUnread', array()), ), true) . '" />
			<br/>
			' . $__templater->button('', array(
				'href' => $__templater->func('link', array('forums/delete-icon-unread', $__vars['node'], ), false),
				'icon' => 'delete',
				'overlay' => 'true',
			), '', array(
			)) . '
...
';
	...
	if (!$__templater->test($__vars['prefixesGrouped'], 'empty', array())) {
		...
		$__compilerTemp9 .= $__templater->formSelectRow(array(
		...
		if ($__templater->isTraversable($__vars['promptGroups'])) {
			foreach ($__vars['promptGroups'] AS $__vars['promptGroupId'] => $__vars['promptGroup']) {
				if ($__vars['promptsGrouped'][$__vars['promptGroupId']]) {
					...
					end($__compilerTemp16); $__compilerTemp17 = key($__compilerTemp16);
					if ($__templater->isTraversable($__vars['promptsGrouped'][$__vars['promptGroupId']])) {
						foreach ($__vars['promptsGrouped'][$__vars['promptGroupId']] AS $__vars['promptId'] => $__vars['prompt']) {
							$__compilerTemp16[$__compilerTemp17]['options'][] = array(
