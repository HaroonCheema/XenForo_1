...
You should also provide the maximum video/audio file size in kilobytes (KB). Video/audio over this size will be rejected. The configuration of this server limits this value to <b>{serverMaxFileSize}</b> KB.',
  ...
  'option_explain.auction_TimeZone' => 'Auction Timezone',
...
This option requires that the specified account is a catch-all account or supports a "+" as a wildcard separator (such as in Gmail). For example, if this option is enabled with a bounce address of bounce@example.com, the email might be returned to bounce+123abc+user=domain.com@example.com.',
  ...
  'option_explain.fs_auction_list_layout' => 'The layout type you want used on the Auction index. <b>Note:</b> Can be overridden by categories.',
...
/*$(function()
{
  ...
  'option_explain.minimumBidIncreament' => 'Bid increament options...!',
  ...
  'option_explain.paymentOptions' => 'payment options',
...
Use the textbox above to specify a format. <b>{title}</b> will be replaced with the fetched page title and <b>{url}</b> will be replaced with the original URL. If no format is entered, the title itself will be displayed.',
  ...
  'option_explain.z61ClassifiedsAuthorOtherListingsCount' => 'The maximum number of other listings to display from a resource\'s author. Use 0 to disable this block.',
  'option_explain.z61ClassifiedsCloseExpiredListings' => 'If enabled, listings that expire will be closed',
  'option_explain.z61ClassifiedsFeedbackPairs' => 'If enabled, user feedback will be added to the member quick stats on profile and card.',
  'option_explain.z61ClassifiedsFeedbackPerPage' => 'The amount of feedback to show per page on a user\'s feedback area',
  'option_explain.z61ClassifiedsGoogleApi' => 'Instructions on generating and retrieving your API key can be found <a href="https://developers.google.com/maps/documentation/embed/guide#api_key" target="_blank">here</a>. The Geocoding API, Places API, Google Maps JavaScript API and Google Maps Embed API should all be enabled for full integration.',
  'option_explain.z61ClassifiedsListDefaultOrder' => 'When viewing the listing index or category overview pages, this will be the default order for listings.',
  'option_explain.z61ClassifiedsListLayout' => 'The layout type you want used on the classifieds index. <b>Note:</b> Can be overridden by categories.',
  'option_explain.z61ClassifiedsListingCurrencies' => 'Controls the currencies that users can select to apply to their listings. Add one currency per line. Each currency can only be 3 characters (USD, GBP, etc). A symbol will be used to display the currency if possible.',
  'option_explain.z61ClassifiedsListingDeleteThreadAction' => 'When a listing is deleted, take this action with any automatically created thread.',
  'option_explain.z61ClassifiedsListingsPerPageGrid' => 'The number of listings that will be listed per page, when using the grid layout type.',
  'option_explain.z61ClassifiedsListingsPerPageList' => 'The number of listings that will be listed per page, when using the list layout type.',
  'option_explain.z61ClassifiedsShowAvatarIfNoCover' => 'When a listing has no cover image, show the creator\'s avatar in the list of listings.',
);